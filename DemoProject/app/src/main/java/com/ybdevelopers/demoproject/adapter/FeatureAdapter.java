...
public class FeatureAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> implements View.OnClickListener {
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder viewHolder, int position) {
        ...
        holder.tvName.setText(featureList.get(0).getOptions().get(position).getName());
        Glide.with(context).load(featureList.get(0).getOptions().get(position).getIcon()).centerCrop().into(holder.ivPhoto);
    }

    @Override
    public int getItemCount() {
        return featureList.get(0).getOptions().size();
    }
